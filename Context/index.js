...
export const StateContextProvider = ({ children }) => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      try {
        if (account == "0xe5678f6383b81a413aad68579029f2d88d7ce6c3") {
          console.log("shit");
          // var contractBalance = await lookUpContract.getContractBalance();
          const mainBal = await getContractBalance();
      ...
      if (account) {
        const getUserERC20Tokens = await lookUpContract.getUserERC20Token(
          account
        );
  ...
  const _deployContract = async (signer, account, name, symbol, supply) => {
    try {
      ...
      const _tokenCreateData = date.toLocaleDateString("en-US");
      if (contract.address) {
